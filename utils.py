...
class GreedySearchEncoderDecoderOutput(ModelOutput):
    ...
    cross_attentions: Optional[Tuple[Tuple[torch.FloatTensor]]] = None
    decoder_hidden_states: Optional[Tuple[Tuple[torch.FloatTensor]]] = None
    all_EElayer_exits: Optional[Tuple[Tuple[torch.FloatTensor]]] = None
...
class GenerationMixin:
    ...
    def generate(
        ...
        **model_kwargs,
        ...
        bos_token_id = bos_token_id if bos_token_id is not None else self.config.bos_token_id
        # print("self.config.num_beams",self.config.num_beams)
    ...
    def greedy_search(
        ...
        **model_kwargs,
        ...
        ```"""
        # print("output_layer1111",output_layer)
        # init values
        logits_processor = logits_processor if logits_processor is not None else LogitsProcessorList()
        stopping_criteria = stopping_criteria if stopping_criteria is not None else StoppingCriteriaList()
        if max_length is not None:
        ...
        this_peer_finished = False  # used by synced_gpus only
        exit_layer_counter = {(i+1):0 for i in range(self.config.decoder_layers)}
        exit_layer_list=[]
        while True:
            if synced_gpus:
                # Under synced_gpus the `forward` call must continue until all gpus complete their sequence.
            ...
            # exit_layer_list.append()
            # print("exit_laye33",exit_layer)
            ...
            # print("next_tokens_scores",next_tokens_scores.shape)
        ...
        if return_dict_in_generate:
            ...
            return input_ids, exit_layer_counter
